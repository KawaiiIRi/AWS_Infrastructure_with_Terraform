...
variable "container_definition" {
  type        = string
  description = "タスクのコンテナ定義(JSON文字列)"
  default     = <<JSON
[
  {
    "name": "nginx",
...
JSON
  ...
  validation {
    condition     = can(jsondecode(var.container_definition))
    error_message = "container_definition must be JSON format"
  }
}
